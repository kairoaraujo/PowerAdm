...
try:
    ...
    if sys.argv[1] == "-sn":
        ...
        if len(sys.argv) < 3:
            print('-sp requires the position in array. Use -sn to show number '
                  'of systems')
            exit(1)
        else:
            try:
...
###############################################################################
        ...
        """ List shared storage pools array size (number of shared storage
        pools)"""
        ...
        if len(sys.argv) < 3:
            print('-poolp requires the position in array. Use -pooln to show '
                  'number of systems')
            exit(1)
        else:
            try:
                ...
                print('The -poolp [position] is a number existent on the '
                      'array.\n'
...
###############################################################################
        ...
        if len(sys.argv) < 3:
            print('-vswp requires the position in array. Use -vswn to show '
                  'number of systems')
            exit(1)
        else:
            try:
                ...
                print('The -vswp [position] is a number existent on the '
                      'array.\n'
...
###############################################################################
        ...
        """ Print lsnports and NPIV Notes (if you are using) from VIOS #1 of
        NPIV. """
        ...
        """ Print lsnports and NPIV Notes (if you are using) from VIOS #2 of
        NPIV. """
        ...
        if len(sys.argv) < 4:
            print('-npiv1p [system] [position]. Requeries the name of system '
                  'and position.')
        else:
            npivs = npiv.NPIV()
        ...
        if len(sys.argv) < 4:
            print('-npiv1 [system] [position]. Requeries the name of system '
                  'and position.')
        else:
            npivs = npiv.NPIV()
...
###############################################################################
        ...
        vfc = sys.argv[13]
        npiv_vios1 = sys.argv[14]
        npiv_vios2 = sys.argv[15]
        ...
        if config.api_debug == "yes":
            print('The parameters send is:\n'
                  ...
                  'System host: %s\n'
                  'Action: %s\n' %
                  (change, prefix, lparname, nim_deploy, lparmem, lparentcpu,
                   lparvcpu, vscsi, add_disk, stgpool, disk_size, vfc,
                   npiv_vios1, npiv_vios2, vlan_deploy, vsw_deploy, net_vlan1,
                   net_vsw1, net_vlan2_1, net_vsw2_1, net_vlan2_2, net_vsw2_2,
                   net_vlan3_1, net_vsw3_1, net_vlan3_2, net_vsw3_2,
                   net_vlan3_3, net_vsw3_3, net_length, system_option, action))
        ...
        if config.vslot_std:
            ...
            lparid = nextid.mkid(system_option)

        # get system VIOs
        listvios = systemvios.SystemVios()
        vios1 = listvios.returnVio1(system_option)
        vios2 = listvios.returnVio2(system_option)
        ...
        if nim_deploy == 'y':
            if net_length == 1:
                ...
                veth_final = ("10/0/%s//0/0/%s,11/0/%s//0/0/%s"
                              % (net_vlan2_1, net_vsw2_1, net_vlan2_2,
                                 net_vsw2_2))
            elif net_length == 3:
                ...
                veth_final = ("10/0/%s//0/0/%s,11/0/%s//0/0/%s,"
                              "12/0/%s//0/0/%s"
                              % (net_vlan3_1, net_vsw3_1, net_vlan3_2,
                                 net_vsw3_2, net_vlan3_3, net_vsw3_3))
        else:
        ...
        print('Config validation: \n\n'
              ...
              (change, prefix, lparname, lparid, nim_deploy, lparmem,
               ...
               npiv_vios1, npiv_vios2, veth, veth_final, system_option,
               vios1, vios2))
        print("*"*80)
        ...
        if action == "create":
            ...
            """ Make a file change """
            newchange = mklparconf.MakeLPARConf(change, prefix, lparname,
                                                lparid, nim_deploy, lparmem,
                                                lparentcpu, lparvcpu, vscsi,
                                                add_disk, stgpool, disk_size,
                                                vfc, npiv_vios1, npiv_vios2,
                                                veth, veth_final,
                                                system_option, vios1, vios2)
...
# Avaiable LPAR configs to deploy
        ...
        if len(sys.argv) < 3:
            print('-osp requires the position in array. Use -osn to show '
                  'number of config deploys')
            exit(1)
        else:
            try:
                print(deploy_list[int(sys.argv[2])])
            except(IndexError, ValueError):
                print('The -osp [position] is a number existent on the '
                      'array.\n'
...
# Get NIM Server OS Versions available
        ...
        """ Print OS NIM Deploys version in array. Use osvl to show number of
        OS version """

        if len(sys.argv) < 3:
            print('-osvp requires the position in array. Use -osvn to show '
            ...
            try:
                ...
                print('The -osvp [position] is a number existent on the '
                      'array.\n'
                      'The number starts with 0.\n'
...
# Get NIM Server informations
        ...
        """ Print OS NIM Server in array. Use -nimn to show number of NIM
        Servers """

        if len(sys.argv) < 3:
            print('-nimp requires the position in array. Use -nimn to show '
                  'number available size')
            exit(1)
        else:
            try:
                ...
                print('The -nimp [position] is a number existent on the '
                      'array.\n'
                      'The number starts with 0.\n'
...
# Deploy OS
        ...
        """ Deploy OS using NIM.
            ...
            apimain.py -nimosdeploy 'NIM File' 'NIM OS Version' 'NIM Server'
            'y or n'
            ...
            Sample:
            apimain.py -nimdeploy 'foo-bar.nim' 'AIX 7.1 TL03 SP04' 'nimsrv01'
            'y'
        """

        if len(sys.argv) < 5:
            print('-nimdeploy requires \'NIM File\' \'NIM OS Version\' '
                  '\'NIM Server\' \'y or n\'\n'
                  'Use -h to help')
            exit(1)
        else:
            ...
            if deploy == 'n':
                ...
                print('Config verification:\n\n'
                      ...
                      'Deploy: %s\n'
                      % (lparprefix, lparname, lparframe, lparvlans, nim_file,
                         nim_cfg_ver, nim_cfg_spot, nim_cfg_mksysbspot,
                         nim_address, nim_ipstart, nim_ipend, nim_ipnet,
                         nim_server, nim_ipdeploy, deploy))

            else:
                """ If deploy is 'y' make deploy """
                deploy_os = mkosdeploy.MakeNIMDeploy(lparprefix, lparname,
                                                     lparframe, lparvlans,
                                                     nim_file, nim_cfg_ver,
                                                     nim_cfg_spot,
                                                     nim_cfg_mksysbspot,
                                                     nim_address, nim_ipstart,
                                                     nim_ipend, nim_ipnet,
                                                     nim_server, nim_ipdeploy,
                                                     deploy)
                """ Start deployment """
...
###############################################################################
        ...
        print('%s is part of PowerAdm %s - http://www.poweradm.org\n'
              'This is a project of API for Web Interface and VMware vCenter '
              'Orchestrator.\n\n'
              'usage: %s [options]\n\n'
              'Arguments:\n'
              '-sn \t\t\t\tList systems array size (number of systems).\n'
              '-sp [position] \t\t\tPrint system position name in array.\n'
              ' \t\t\t\tPosition in array. Position start with 0. Use -sn to '
              'check array size.\n'
              '-nimstatus \t\t\tReturn NIM enable status [yes|no].\n'
              '-sspstatus \t\t\tReturn Shared Storage Pool enable status '
              '[yes|no].\n'
              '-pooln \t\t\t\tList shared storage pools array size (number of '
              'shared storage pools).\n'
              '-poolp [positon] \t\tPrint shared storage pool position name in'
              ' array.\n'
              ' \t\t\t\tPosition in array. Position start with 0. Use -pooln '
              'to check array size.\n'
              '-vswn \t\t\t\tList Virtual Switches array size (number of '
              'virtual switches).\n'
              '-vswp [postion] \t\tPrint virtual switch position name in'
              ' array.\n'
              ' \t\t\t\tPosition in array. Position start with 0. Use -vswn to'
              ' check array size.\n'
              '-npiv1 [system] \t\tPrint informations about NPIV (lsmap and'
              ' notes) on Primary NPIV VIOS.\n'
              ' \t\t\t\tRequeries the name of system.\n'
              '-npiv2 [system] \t\tPrint informations about NPIV (lsmap and '
              'notes) on Secondary NPIV VIOS.\n'
              ' \t\t\t\tRequeries the name of system.\n'
              '-npiv1n [system] \t\tList FCs array size (number of FCs'
              ' available) on Primary NPIV VIOS.\n'
              ' \t\t\t\tRequeries the name of system\n'
              '-npiv1p [system] [position]\tPrint FC name in specific'
              ' position\n'
              ' \t\t\t\tRequeries the name of system\n'
              ' \t\t\t\tPosition start with 0. Use -npiv1n to check array'
              ' size.\n'
              '-npiv2n [system] \t\tList FCs array size (number of FCs'
              ' available) on Secondary NPIV VIOS.\n'
              ' \t\t\t\tRequeries the name of system.\n'
              '-npiv2p [system] [position]\tList FCs array size (number of FCs'
              ' available) on Secondary NPIV VIOS.\n'
              ' \t\t\t\tRequeries the name of system.\n'
              ' \t\t\t\tPosition start with 0. Use -npiv2n to check array'
              ' size.\n'
              '-osn \t\t\t\tGet array size of available files/LPAR to NIM'
              ' deploy.\n'
              '-osp [position] \t\tPrint config deploy [position] in array.\n'
              ' \t\t\t\tPosition in array. Position start with 0. Use -osn to'
              ' check array size.\n'
              '-osvn \t\t\t\tGet array size of available OS versions on NIM'
              ' Deploy\n'
              '-osvp [position] \t\tPrint OS NIM Deploys version in array. '
              'Use osvl to show number of OS version\n'
              '-nimn \t\t\t\tGet array size of available NIM Servers\n'
              '-nimp [position] \t\tPrint OS NIM Server in array.\n'
              ' \t\t\t\tPosition in array. Position start with 0. Use -nimn '
              'to check array size.\n'
              '-mklparcfg [arguments] \t\tCreate LPAR\n'
              '\t\t\t\tArguments in order:\n'
              '\t\t\t\tchange prefix lparname nim_deploy lparmem lparentcpu '
              'lparvcpu vscsi add_disk stgpool\n'
              '\t\t\t\tdisk_size vfc npiv_vios1 npiv_vios2 vlan_deploy vsw_'
              'deploy net_vlan1 net_vlan2_1 net_vlan2_2\n'
              '\t\t\t\tnet_vlan3_1 net_vlan3_2 net_vlan3_3 net_vsw1 net_'
              'vsw2_1 net_vsw2_2 net_vsw3_1 net_vsw3_2 net_vsw3_3\n'
              '\t\t\t\tnet_length system_option action\n'
              '\t\t\t\tCheck the documentation about API in '
              'http://poweradm.org/apimain.html\n'
              '-nimdeploy [arguments] \t\tDeploy OS using NIM. '
              'Requires arguments.\n'
              '\t\t\t\tArguments in order:\n'
