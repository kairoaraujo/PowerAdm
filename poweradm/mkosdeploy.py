...
class MakeNIMDeploy():
    """ Make Operation System NIM Deploy
        ...
        lparvlans            LPAR VLAN Final configuration - from nim see
                                  NIMFileFind()
        nim_cfg_ver          NIM Config OS Version - from nim see NIMGetVer()
        nim_cfg_spot         NIM Config SPOT - from nim see NIMGetVer()
        nim_cfg_mksysbspot   NIM Config MKSYSB or SPOT - from nim see
                                 NIMGetVer()
        new_ip               New IP used by LPAR client - from nim see
                                 NIMNewIP()
        nim_address          NIM Server IP Address/hostname - from nim see
                                 NIMServer()
        ...
        nim_ipdeploy         NIM Server IP of nim net - from nim see
                                 NIMServer()
        deploy               Deploy action (yes / no )
    """

    def __init__(self, lparprefix, lparname, lparframe, lparvlans, nim_file,
                 nim_cfg_ver, nim_cfg_spot, nim_cfg_mksysbspot, nim_address,
                 nim_ipstart, nim_ipend, nim_ipnet, nim_server, nim_ipdeploy,
                 deploy):
        ...
        if config.nim_bosinst_data_res == '':
            ...
            self.bosinst_data = '-a bosinst_data=%s' % \
                                config.nim_bosinst_data_res
